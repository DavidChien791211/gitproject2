...
function makeHeader(blog_username, username){
    ...
    //訪問部落格相簿頁面
    var user_photo_url = '/' + blog_username + '/'+'photo'
    //访问博主的博客文章
    ...
    if (blog_username == ""){
        ...
        var get_url_name = username.split("/")[0];
        header_body += '<li><a href=/' + get_url_name + '/user_index>網站首頁</a></li>';
    }
    header_body += '<li>';
    ...
    header_body += '</ul>';
    header_body += '</li>';
//    header_body += '<li><a href="photo.html">我的相簿</a> </li>';
    header_body += '<li><a href=' + '"' + user_photo_url + '"' + '>我的相簿</a> </li>';
    header_body += '<li><a href=' + '"' + user_info_url + '"' + '>關於我</a> </li>';
    header_body += '<li><a href=' + '"' + topic_release_url + '"' + '>發表部落格</a> </li>';
    header_body += '</nav>';
    header_body += '</div>';
    if (username){
        header_body += '<li><a href= /' + username + '/change_info id="change_info" target="_blank">編輯</a></li>';
        //header_body += '<li><a href="/" id="login_out" target="_blank">登出</a></li>';
        header_body += '<li><span id="login_out" target="_blank">登出</span></li>';
    }else{
        header_body += '<a href="/login" id="login" target="_blank">登錄</a>';
